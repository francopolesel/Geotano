...
Client (`useTrucoMultiplayer`): TanStack Query owns server state; pushes trigger invalidation only; benign lost-race rejections — 409 CAS conflicts and engine `E_OUT_OF_TURN` / `E_STATE_FORBIDDEN` from a rival who moved concurrently — all recover the same way: refetch authority instead of mutating locally, with a calm "syncing" banner. Genuinely hostile errors (e.g. `E_CARD_NOT_OWNED`, 403, 404) surface as explicit errors. If a background refresh fails while a cached board is on screen, the page marks it stale/reconnecting instead of silently rotting. Poll fallback lives ON THE QUERY as `refetchInterval: 10_000` (S5 decision — deliberately not a raw `setInterval`; do not revert). Reloading restores the exact position from GET — including a pending truco answer — and there is no auto-forfeit. Unfinished/finished rows are swept hourly after 24 h (`deleteExpiredTrucoMatches`).
