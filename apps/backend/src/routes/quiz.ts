...
export async function quizRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      try {
        const { sessionId, question, totalQuestions, maxLives } = await startSession(userId, mode as GameModeSlug, lang || 'en');
        return { sessionId, question, totalQuestions, maxLives };
      } catch (err) {
